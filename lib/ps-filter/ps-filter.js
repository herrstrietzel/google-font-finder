...
function getfilterInputsFromObj(props, radios = ['category'], presets = {}, exclude = [], addLabels = true) {
    ...
    for (let prop in props) {
        ...
        //add labels
        if (addLabels) checkboxesHtml += `<label>${prop}</label> `;

        vals.forEach(val => {

            //filterClassCache
            let checked = presets[prop] ? presets[prop].includes(val) : false;
